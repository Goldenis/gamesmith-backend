...
trait AdminController {
  ...
  def sudoUpdateEmail = silhouette.SecuredAction(SuperUserAuthorizationCheck) { implicit request =>
    emailForm.bindFromRequest()(request).value.map { case (id, email) =>
      if (databaseService.updateEmail(id, email))
        NoContent
      else
  ...
  def downloadUserPrefs = silhouette.SecuredAction(SuperUserAuthorizationCheck) { implicit request =>
    ...
    val csvHeader = Iterator.single("Name,Email,Job Title,Phone Number,Availability,Company,Location,Member Connections,Member Messages,Surveys,Job Enquiries\n")
    val csvLines = users.map { case (account, settings) =>
      csvFormat.format(account.profile.fullName, account.email,account.maker.get.currRole, account.profile.phoneNumber.getOrElse(""),account.profile.availability.getOrElse(""),account.maker.get.currCompany,account.profile.location.getOrElse(""), settings.memberConnections.toString,
